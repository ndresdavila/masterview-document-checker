...
const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../random-10f')
...
for (const id of ids) {
  ...
  console.log(`net P:${proforma.totals?.netKg} H:${hbl.totals?.netKg}  gross P:${proforma.totals?.grossKg} H:${hbl.totals?.grossKg}`)
  for (const item of mismatches) {
    const p = String(item.proforma).replace(/\s+/g, ' ').slice(0, 140)
    const h = String(item.hbl).replace(/\s+/g, ' ').slice(0, 140)
