...
const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../random-10g')
...
for (const id of ids) {
  ...
  for (const item of mismatches) {
    const p = String(item.proforma).replace(/\s+/g, ' ').slice(0, 160)
    const h = String(item.hbl).replace(/\s+/g, ' ').slice(0, 160)
