...
const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '../random-15')
...
for (const id of ids) {
  ...
  const hblName = files.find((f) => /\.pdf$/i.test(f))
  ...
  const hbl = await parsePdf(path.join(dir, hblName))
  ...
  console.log(`ctr P:${proforma.containers.length}  H:${hbl.containers.length}`)
  ...
  for (const item of warnings.slice(0, 4)) {
    console.log(`  [warning] ${item.label} — ${item.detail}`)
  }
  if (warnings.length > 4) console.log(`  [warning] … +${warnings.length - 4} avisos`)
}

console.log('\n===== RESUMEN =====')
for (const row of summary) {
  console.log(`${row.id}  ${row.score}%  mismatch=${row.mismatch}  warn=${row.warn}`)
}
const avg = Math.round(summary.reduce((s, r) => s + r.score, 0) / summary.length)
const clean = summary.filter((r) => r.mismatch === 0).length
console.log(`promedio=${avg}%  sin mismatch=${clean}/${summary.length}`)
